...
static int isotp_send_flow_control(IsoTpLink* link, uint8_t flow_status, uint8_t block_size, uint8_t st_min_ms) {
    ...
    ret = isotp_user_send_can(link, link->send_arbitration_id, message.as.data_array.ptr, sizeof(message));
#else    
    ret = isotp_user_send_can(link, link->send_arbitration_id,
...
static int isotp_send_single_frame(IsoTpLink* link, uint32_t id) {
    ...
    ret = isotp_user_send_can(link, id, message.as.data_array.ptr, sizeof(message));
#else
    ret = isotp_user_send_can(link, id,
            message.as.data_array.ptr,
            link->send_size + 1);
#endif
...
static int isotp_receive_single_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    if ((0 == message->as.single_frame.SF_DL) || (message->as.single_frame.SF_DL > (len - 1))) {
        isotp_user_debug(link, "Single-frame length too small.");
...
static int isotp_receive_first_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    if (8 != len) {
        isotp_user_debug(link, "First frame should be 8 bytes in length.");
        return ISOTP_RET_LENGTH;
    }
    ...
    if (payload_length <= 7) {
        isotp_user_debug(link, "Should not use multiple frame transmission.");
        return ISOTP_RET_LENGTH;
    }
    
    if (payload_length > link->receive_buf_size) {
        isotp_user_debug(link, "Multi-frame response too large for receiving buffer.");
...
static int isotp_receive_consecutive_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    if (remaining_bytes > len - 1) {
        isotp_user_debug(link, "Consecutive frame too short.");
        return ISOTP_RET_LENGTH;
    }
...
static int isotp_receive_flow_control_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    if (len < 3) {
        isotp_user_debug(link, "Flow control frame too short.");
        return ISOTP_RET_LENGTH;
    }
...
int isotp_send_with_id(IsoTpLink *link, uint32_t id, const uint8_t payload[], uint16_t size) {
    ...
    if (link == 0x0) {
        isotp_user_debug(link, "Link is null!");
        return ISOTP_RET_ERROR;
    }

    if (size > link->send_buf_size) {
        isotp_user_debug(link, "Message size too large. Increase ISO_TP_MAX_MESSAGE_SIZE to set a larger buffer\n");
        char message[128];
    ...
    if (ISOTP_SEND_STATUS_INPROGRESS == link->send_status) {
        isotp_user_debug(link, "Abort previous message, transmission in progress.\n");
    ...
    if (link->send_size < 8) {
        ...
        if (ISOTP_RET_OK == ret) {
            ...
            link->send_timer_st = isotp_user_get_ms(link);
            link->send_timer_bs = isotp_user_get_ms(link) + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
    ...
    switch (message.as.common.type) {
        ...
        case ISOTP_PCI_TYPE_FIRST_FRAME: {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                /* refresh timer cs */
                link->receive_timer_cr = isotp_user_get_ms(link) + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
            }
...
void isotp_init_link(IsoTpLink *link, uint32_t sendid, uint8_t *sendbuf, uint16_t sendbufsize, uint8_t *recvbuf, uint16_t recvbufsize, void* owner) {
...
void isotp_poll(IsoTpLink *link) {
    ...
    if (ISOTP_SEND_STATUS_INPROGRESS == link->send_status) {
        ...
        (0 == link->send_st_min || (0 != link->send_st_min && IsoTpTimeAfter(isotp_user_get_ms(link), link->send_timer_st)))) {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                link->send_timer_bs = isotp_user_get_ms(link) + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
                link->send_timer_st = isotp_user_get_ms(link) + link->send_st_min;
        ...
        /* check timeout */
        if (IsoTpTimeAfter(isotp_user_get_ms(link), link->send_timer_bs)) {
    ...
    if (ISOTP_RECEIVE_STATUS_INPROGRESS == link->receive_status) {
        ...
        /* check timeout */
        if (IsoTpTimeAfter(isotp_user_get_ms(link), link->receive_timer_cr)) {
